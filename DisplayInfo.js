...
class SearchBarFilteredList extends Component {
  constructor(props){
    super(props);
    this.state = {}
  }
...
class Panels extends Component{
  constructor(props){
    var options = [1,2,3];
    super(props);
    this.state = {options:options, selected_option_index: 0}
  }

  toggle(){
    var that = this;
    var new_option = (that.state.selected_option_index + 1)%3
    this.setState({selected_option_index:new_option})
  }

  renderContent(){
    var that = this;
    if(that.state.selected_option_index === 0){
      return ( 
      <View style = {{width:"100%",height:"100%", alignItems:'center', justifyContent:'center', backgroundColor:"rgb(242,242,247)"}}>
        <Text>
    ...
    if(that.state.selected_option_index === 1){
      return ( 
      <View style = {{width:"100%",height:"100%", alignItems:'center', justifyContent:'center', backgroundColor:"rgb(242,242,247)"}}>
        <Text>
           OPTION 2 TEXT GOES HERE
        </Text>
      </View>
              )
    }

    if(that.state.selected_option_index === 2){
      return (
        <View style = {{width:"100%",height:"100%", alignItems:'center', justifyContent:'center', backgroundColor:"rgb(242,242,247)"}}>
        <Text>
           OPTION 3 TEXT GOES HERE
        </Text>
      </View>
  ...
  render(){
    var that = this;
    return (<View style = {{height:"100%", width:"100%", alignItems:'center'}}>
      <View style = {{flexDirection:'row', height:"10%", width:"80%", justifyContent:'space-evenly', alignItems:'space-evenly'}}>
       {that.state.options.map(function(option){
        return (
        <View style = {{height:"100%",width:"30%"}}>
           <Button color = "rgb(10,132,255)" title = {option} onPress = {that.toggle.bind(that)}></Button>
        </View>
  ...
  render() {
    ...
    return (
      <View style={styles.app}>
        <TitleText text = {"DisplayInfo"}></TitleText>
